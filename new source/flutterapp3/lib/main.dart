...
class _LoginpageState extends State<Loginpage> {
  ...
  Future<http.Response> postRequest() async {
    var url = 'https://dsc-receipt.du.r.appspot.com/api/auth/signin/';
...
class _IDPWState extends State<IDPW> {
              ...
              child: Column(
                children: <Widget>[
                  TextField(
                    controller:email,
                    decoration: InputDecoration(
                        labelText: '회원가입시 입력했던 E-mail 입력하세요.',
                        labelStyle: TextStyle(
                            fontFamily: 'Montserrat',
                            fontWeight: FontWeight.bold,
                            color: Colors.grey),
  ...
  Future<http.Response> postRequest() async {
    var url = 'https://dsc-receipt.du.r.appspot.com/api/main/searchpw/';
  ...
  Future<http.Response> postRequestCode() async {
    var url = 'https://dsc-receipt.du.r.appspot.com/api/main/searchpwcode/';
  ...
  Future<void> Select_date(BuildContext context) {
    return showDialog<void>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('날짜 선택',textAlign: TextAlign.center, style: TextStyle(
              fontWeight: FontWeight.bold,
              color: Colors.black,
              fontFamily: 'Montserrat'
          ),),
  ...
  Future scan() async {
    try {
      ...
      if (e.code == BarcodeScanner.CameraAccessDenied) {
        setState(() {
          this.barcode = '카메라 권한이 없습니다!';
        });
      } else {
        setState(() => this.barcode = '알수없는 에러: $e');
      }
    } on FormatException{
...
class _RegisterState extends State<Register> {
  ...
  Future<http.Response> postRequest() async{
    var url = 'https://dsc-receipt.du.r.appspot.com/api/auth/signup/';
...
Future<List<Result>> fetchResults(http.Client client, String date1, String date2) async {
  ...
  if (date1!="" && date2!=""){
    response = await client.get('https://dsc-receipt.du.r.appspot.com/api/main/receipt_list/${username.trim()}/${date1.trim()}/${date2.trim()}');
    print(response.body);
  } else {
    response = await client.get('https://dsc-receipt.du.r.appspot.com/api/main/receipt_list/${username.trim()}/');
  }
...
Future<List<Result>> fetchMonthlyResults(http.Client client, int month) async {
  http.Response response;
  if (month == 1) {
    response = await client.get('https://dsc-receipt.du.r.appspot.com/api/main/receipt_list/${username}/${month}');
  } else if (month == 3) {
    response = await client.get('https://dsc-receipt.du.r.appspot.com/api/main/receipt_list/${username}/${month}');
  } else if (month == 6) {
    response = await client.get('https://dsc-receipt.du.r.appspot.com/api/main/receipt_list/${username}/${month}');
  }
  print('https://dsc-receipt.du.r.appspot.com/api/main/receipt_list/${username}/${Month}');
  return compute(parseResults, response.body);
}

Future<List<MoneyResult>> fetchResultsTotalMoney(http.Client client) async {
  final response =
  await client.get('https://dsc-receipt.du.r.appspot.com/api/main/receipt/'+username.trim()+'/');
